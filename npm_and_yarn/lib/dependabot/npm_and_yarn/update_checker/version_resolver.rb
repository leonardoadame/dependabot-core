    ...
    class UpdateChecker
      class VersionResolver
        ...
        def handle_peer_dependency_errors(error)
          ...
          elsif error.message.match?(PNPM_PEER_DEP_ERROR_REGEX)
            error.message.scan(PNPM_PEER_DEP_ERROR_REGEX) do
        ...
        def run_checker(path:, version:)
          ...
          return run_pnpm_checker(path: path, version: version, lockfile: root_pnpm_lock) if root_pnpm_lock
          ...
          SharedHelpers.with_git_configured(credentials: credentials) do
            Dir.chdir(path) do
              output = SharedHelpers.run_shell_command(
                "pnpm install #{dependency.name}@#{version} --lockfile-only",
                fingerprint: "pnpm install <dependency_name>@<version> --lockfile-only"
              )
